	...
	Effect.fn(function* (handlers) {
		...
		return handlers
			...
			.handle("approveRestaurant", ({ params }) =>
				Effect.gen(function* () {
					const { sub: adminId, role } = yield* AuthContext;
					yield* assertAdminOnly(role);
					const result = yield* admin
						.approveRestaurant(params.id, adminId)
						.pipe(Effect.catchTag("DbError", Effect.orDie));
					yield* cache.invalidateRestaurant(params.id);
					yield* cache.invalidateRestaurantList();
					return result;
				}),
			)
			.handle("rejectRestaurant", ({ params, payload }) =>
				Effect.gen(function* () {
					const { sub: adminId, role } = yield* AuthContext;
					yield* assertAdminOnly(role);
					const result = yield* admin
						.rejectRestaurant(params.id, adminId, payload.reason)
						.pipe(Effect.catchTag("DbError", Effect.orDie));
					yield* cache.invalidateRestaurant(params.id);
					yield* cache.invalidateRestaurantList();
					return result;
			...
			.handle("suspendRestaurant", ({ params, payload }) =>
				Effect.gen(function* () {
					const { sub: adminId, role } = yield* AuthContext;
					yield* assertAdminOnly(role);
					const result = yield* admin
						.suspendRestaurant(params.id, adminId, payload.reason)
						.pipe(Effect.catchTag("DbError", Effect.orDie));
					yield* cache.invalidateRestaurant(params.id);
					yield* cache.invalidateRestaurantList();
					return result;
			...
			.handle("updateCommissionRate", ({ params, payload }) =>
				Effect.gen(function* () {
					const { role } = yield* AuthContext;
					yield* assertAdminOnly(role);
					return yield* admin
						.updateCommissionRate(params.id, payload.commissionRate)
						.pipe(Effect.catchTag("DbError", Effect.orDie));
				}),
			)
			.handle("listUsers", () =>
			...
			.handle("approveDriver", ({ params }) =>
				Effect.gen(function* () {
					...
					yield* assertAdminOnly(role);
					yield* admin
						.approveDriver(params.id, adminId)
						.pipe(Effect.catchTag("DbError", Effect.orDie));
					return {
			...
			.handle("rejectDriver", ({ params, payload }) =>
				Effect.gen(function* () {
					...
					yield* assertAdminOnly(role);
					yield* admin
						.rejectDriver(params.id, adminId, payload.reason)
						.pipe(Effect.catchTag("DbError", Effect.orDie));
					return {
