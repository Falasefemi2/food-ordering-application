...
export const RestaurantLive = Layer.effect(
	...
	Effect.gen(function* () {
		...
		const assertGroupBelongsToItem = (
			groupId: string,
			itemId: string,
		) =>
			Effect.gen(function* () {
				const [row] = yield* dbQuery(
					db
						...
						.where(
							and(
								eq(
									customizationGroups.id,
									groupId,
								),
								eq(
									customizationGroups.menuItemId,
									itemId,
								),
							),
						)
		...
		const updateRestaurant: RestaurantServiceShape["updateRestaurant"] =
			(restaurantId, ownerId, input) =>
				Effect.gen(function* () {
					const current = yield* assertOwner(
						ownerId,
						restaurantId,
					);
					if (
		...
		const updateCategory: RestaurantServiceShape["updateCategory"] =
			(restaurantId, categoryId, ownerId, input) =>
				Effect.gen(function* () {
					yield* assertOwner(
						ownerId,
						restaurantId,
		...
		const deleteCategory: RestaurantServiceShape["deleteCategory"] =
			(restaurantId, categoryId, ownerId) =>
				Effect.gen(function* () {
					yield* assertOwner(
						ownerId,
						restaurantId,
		...
		const createCustomizationGroup: RestaurantServiceShape["createCustomizationGroup"] =
			(restaurantId, itemId, ownerId, input) =>
				Effect.gen(function* () {
					...
					yield* assertItemBelongsToRestaurant(
						itemId,
						restaurantId,
					);
					const [row] = yield* dbQuery(
		...
		const updateCustomizationGroup: RestaurantServiceShape["updateCustomizationGroup"] =
			(restaurantId, itemId, groupId, ownerId, input) =>
				Effect.gen(function* () {
					...
					yield* assertGroupBelongsToItem(
						groupId,
						itemId,
					);
					const [updated] = yield* dbQuery(
									...
									.select(
										{
											...
											minSelectable:
												customizationGroups.minSelectable,
											maxSelectable:
												customizationGroups.maxSelectable,
									...
									.select(
										{
											...
											isAvailable:
												customizationOptions.isAvailable,
										},
									)
									.from(
										customizationOptions,
